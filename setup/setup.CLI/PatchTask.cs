...
namespace Terraria.ModLoader.Setup
{
	public class PatchTask : SetupOperation
	{
		...
		{
			return taskInterface.RequestConfirmation("confirm-loss", "Any changes in /" + patchedDir + " that have not been converted to patches will be lost.");
		}
		...
		private void ShowReviewWindow(IEnumerable<FilePatcher> results) {
			throw new NotImplementedException(); //TODO: Update review window
			taskInterface.DisplayPatchReviewer(results, baseDir + '/');
		...
		public override void FinishedDialog() {
			...
			if(Failed())
				taskInterface.DisplayError($"Patches applied with {failures} failures and {warnings} warnings.\nSee /logs/patch.log for details");
			else
				taskInterface.DisplayWarning($"Patches applied with {failures} failures and {warnings} warnings.\nSee /logs/patch.log for details");
		}
